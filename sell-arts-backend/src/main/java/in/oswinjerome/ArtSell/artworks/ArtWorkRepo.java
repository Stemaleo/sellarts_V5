...
public interface ArtWorkRepo extends JpaRepository<ArtWork, String> {
    ...
    @Query("SELECT e FROM art_works e WHERE e.is_deleted = false AND e.owner.is_deleted = false AND e.paintingType.id NOT IN (159, 202, 203, 204, 205) ORDER BY RAND() LIMIT 5")
