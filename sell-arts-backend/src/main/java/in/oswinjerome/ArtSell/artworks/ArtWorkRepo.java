...
public interface ArtWorkRepo extends JpaRepository<ArtWork, String> {
    ...
    @Query("SELECT a FROM art_works a WHERE a.owner = :owner AND a.is_deleted = false")
    ...
    @Query("SELECT a FROM art_works a WHERE a.owner = :owner AND a.is_deleted = false ORDER BY a.createdAt ASC")
    Page<ArtWork> findAllByOwnerOrderByCreatedAtAsc(User owner, Pageable pageable);

    @Query("SELECT a FROM art_works a WHERE a.owner = :owner AND a.is_deleted = false ORDER BY a.updatedAt DESC")
    ...
    @Query("SELECT COUNT(a) FROM art_works a WHERE a.owner = :owner AND a.is_deleted = false")
    Long countByOwner(User owner);

    @Query("SELECT new in.oswinjerome.ArtSell.favourites.FavStatsDTO(a,COUNT (f)) FROM art_works a JOIN Favourite f ON f.artWork = a WHERE a.is_deleted = false GROUP BY a ORDER BY COUNT(f) DESC")
    List<FavStatsDTO> getArtWorkWithFavStats();

    @Query("SELECT COUNT(a) FROM art_works a WHERE a.is_deleted = false AND a.createdAt BETWEEN :start AND :end")
    Long countByCreatedAtBetween(LocalDateTime start, LocalDateTime end);

    @Query("SELECT COUNT(a) FROM art_works a WHERE a.owner = :owner AND a.is_deleted = false AND a.createdAt BETWEEN :createdAt AND :createdAt2")
