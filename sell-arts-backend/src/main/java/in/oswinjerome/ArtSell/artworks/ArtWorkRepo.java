...
public interface ArtWorkRepo extends JpaRepository<ArtWork, String> {
    ...
    @Query("SELECT art FROM art_works as art WHERE art.owner = :owner AND art.title LIKE %:title% AND CONCAT('',art.paintingType.id) LIKE LOWER(CONCAT('%', :paintingType, '%')) AND art.size LIKE %:size%")
    Page<ArtWork> findAllByOwnerAndSize(User owner, String title, String paintingType, String size, Pageable pageable);

    @Query("SELECT art FROM art_works as art WHERE art.title LIKE %:title% AND CONCAT('',art.paintingType.id) LIKE LOWER(CONCAT('%', :paintingType, '%')) AND art.size LIKE %:size%")
    Page<ArtWork> findAllWithSize(String title, String paintingType, String size, Pageable pageable);
    ...
    @Modifying
    @Transactional
    @Query("UPDATE art_works at SET at.paintingType = :newType WHERE at.paintingType = :oldType")
    ...
    @Query("SELECT new in.oswinjerome.ArtSell.favourites.FavStatsDTO(a,COUNT (f)) FROM art_works a JOIN Favourite f ON f.artWork = a GROUP BY a ORDER BY COUNT(f) DESC")
