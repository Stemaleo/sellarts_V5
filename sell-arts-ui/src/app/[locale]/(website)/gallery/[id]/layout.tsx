...
export default async function ArtistPage({ params, children }: PageProps & { children: ReactNode }) {
  ...
  const navItems = [
    { name: "Art Works", href: `/artists/${param.id}`, show: true },
    { name: "Collaborators", href: `/artists/${param.id}/collaborators`, show: artist.artistProfile?.artistType === "GALLERY" },
    { name: "Events", href: `/artists/${param.id}/events`, show: true },
  ];
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid gap-8 mt-20">
        <div>
          ...
          <div className="flex gap-4 mb-4 mt-4">
            ...
            <Button size={"sm"}>{t("common.subscribe")}</Button>
          </div>
          {artist.artistProfile?.bio && <h2 className="text-2xl font-semibold mb-2">Biography</h2>}
      ...
      <div className="p-4 mb-6 mt-12 bg-primary rounded flex items-baseline space-x-4 text-white">
        {navItems
          .filter((c) => c.show)
          .map((item) => (
            <Link key={item.name} href={item.href} className={`group relative px-3 py-2 text-sm font-medium ${true ? "text-white" : "text-gray-600"} transition-colors duration-200`}>
              {item.name}
              {true && <span className="absolute opacity-0 group-hover:opacity-100 -bottom-4 left-0 w-full h-1 bg-white transition-all duration-200"></span>}
            </Link>
          ))}
      </div>
