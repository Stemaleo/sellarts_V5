...
const buttonVariants = cva("inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50", {
  variants: {
    variant: {
      default: "bg-primary isolate bg-pattern text-primary-foreground shadow hover:bg-primary/90",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant, size, asChild = false, loading = false, ...props }, ref) => {
  ...
  return (
    <Comp className={"relativse " + cn(styles) + " "} ref={ref} {...props}>
      <div className="flex gap-2 items-center relative justify-center">
