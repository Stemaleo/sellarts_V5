...
import { updateFeaturedStatusOfArtWork } from "@/actions/artwork";
...
const ArtCard = ({ artwork }: { artwork: ArtWorkDTO }) => {
  ...
  return (
    <div key={artwork.id} className="bg-white rounded-lg shadow-md overflow-hidden relative">
      {artwork.inStock ? <Badge className="absolute z-10 top-2 right-2 bg-green-500">In stock</Badge> : <Badge className="absolute z-10 top-2 right-2 bg-red-500">Sold</Badge>}
      <div className="relative h-48">
        <Image src={artwork.mediaUrls[0]} alt={artwork.title} layout="fill" objectFit="cover" />
      </div>
      <div className="p-4">
        <div className="flex justify-between items-center">
          <span className="text-xs text-gray-500 bg-primary/20 py-1 px-2 rounded-full inline h-auto">By, {artwork.ownerName}</span>
