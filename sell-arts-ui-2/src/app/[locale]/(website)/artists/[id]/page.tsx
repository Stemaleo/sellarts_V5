...
export default async function ArtistPage(props: PageProps) {
  ...
  const res = await getArtistProfileWithInfo(params.id);
  ...
  return (
    <div className="">
      <h2 className="text-3xl font-bold mt-12 mb-6">{t("artistsPage.Artworks")}</h2>
      {artWorks.length === 0 && <p className="text-center text-gray-500 mt-8">{t("artistsPage.no-artwork-found")}.</p>}
